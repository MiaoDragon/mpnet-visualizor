...
obc,obstacles, paths, path_lengths= None, None, None, None
...
def feasibility_check_2(path,pl,idx):
...
def is_reaching_target(start1,start2):
    ...
    for i in range(0,2):
        if abs(s1[i]-s2[i]) > 2.0:
            return False
    return True

def is_reaching_target_2(start1,start2):
    s1=np.zeros(2,dtype=np.float32)
    s1[0]=start1[0]
    s1[1]=start1[1]
    ...
    for i in range(0,len(path)-1):
        ...
        if steer==1:
            ...
            while (not target_reached) and itr<50 :
                ...
                st=mlp(ip)
                st=st.data.cpu()
                target_reached=is_reaching_target_2(st,gl)
                #if target_reached:
                    #new_path.append(st)
...
def re_iterate_path(p,g,idx,obs):
    step=0
    path=[]
    path.append(p[0])
    for i in range(1,len(p)-1):
            ...
            if path_lengths[i][j]>0:
                ...
                while target_reached==0 and step<80 :
                    ...
                    if tree==0:
                        ...
                        path2.append(start2)
                        tree=0
                    target_reached=steerTo(start1,start2,i);#is_reaching_target(start1,start2)
                tp=tp+1
                indicator3=feasibility_check_2(paths[i][j],path_lengths[i][j],i)
                if indicator3==1:
                    tr=tr+1
                if target_reached==1:
                    ...
                    indicator2=collision_check(path,i)
                    if indicator2==1:
                        cr=cr+1
                    if indicator==1:
                        fp=fp+1
                        ...
                        while indicator==0 and sp<10 and path !=0:
                            ...
                            path=re_iterate_path(path,g,i,obs)
                            if path !=0:
                                path=lvc(path,i)
                                indicator=feasibility_check(path,i)
